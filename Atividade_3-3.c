...
int numClicks = 0; // Contador de cliques para definir os vértices do triângulo
int lineThickness = 1; // Espessura inicial do traçado
...
void drawLine(int x1, int y1, int x2, int y2) {
    ...
    glLineWidth(lineThickness); // Define a espessura do traçado
    glBegin(GL_LINES);
    glVertex2i(x1, y1);
    glVertex2i(x2, y2);
    glEnd();
...
void keyboard(unsigned char key, int x, int y) {
    if (key == 'r' || key == 'R') {
        ...
        // Redefine a espessura do traçado para 1 quando a função de cor está ativa
        lineThickness = 1;
    }
    glutPostRedisplay();
}
